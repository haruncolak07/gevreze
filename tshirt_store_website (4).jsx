            ...
            <div className="flex items-center gap-4 w-full sm:w-auto">
              <div className="relative flex-grow">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-2 border rounded-xl text-sm focus:outline-none w-full"
                />
              </div>
              <Button className="rounded-full px-4 py-2 flex items-center gap-2">
                <span className="material-icons">shopping_cart</span> Sepetim
              </Button>
            </div>
          </header>
            ...
            {filteredProducts.map((product) => (
              <Card key={product.id} className="hover:shadow-xl transition">
                <CardContent className="flex flex-col items-center text-center">
                  <img src={product.image} alt={product.name} className="w-40 h-40 object-cover mb-4 rounded-xl" onError={(e)=>{e.target.src='/placeholder.png'}} />
